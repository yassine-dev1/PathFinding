...
import { loadDijkstraWasm } from "./loadDijkstra";
...
function App() {
  ...
  // function for trace  path result  in grid
  const TracePathInGrid = useCallback(async (path, visitedNodes) => {

    if (!Array.isArray(path) || path.length === 0) {
      alert("Chemin vide ou invalide");
      console.warn("Path reçu :", path);
      return;
    }

    if (!Array.isArray(visitedNodes) || visitedNodes.length === 0) {
      alert("Chemin du node visite est vide ou invalide");
      console.warn("Path du node visite reçu :", visitedNodes);
    }

    console.log("Début tracé du chemin :", path);
    ...
    if (visitedNodes && visitedNodes.length > 0) {
      for (let i = 0; i < visitedNodes.length; i++) {
        const { row, col } = getRowCol(visitedNodes[i]);
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (typeof index !== 'number' || isNaN(index) || index < 0 || index >= ROWS * COLS) {
        console.warn(`Index invalide dans le chemin à la position ${i} :`, index);
        continue;
      }

      const { row, col } = getRowCol(index);

      // Vérification que row/col sont valides
      if (typeof row !== 'number' || isNaN(row) || row < 0 || row >= ROWS ||
        typeof col !== 'number' || isNaN(col) || col < 0 || col >= COLS) {
        console.warn(`Coordonnées invalides pour index ${index} → row=${row}, col=${col}`);
        continue;
      }
      ...
      // Mise à jour progressive (tous les nœuds ou tous les 5 pour plus de fluidité)
    ...
    console.log("Tracé du chemin terminé");
  }, [grid, startNode, endNode]);   // Ajoute les dépendances correctes
